...
class mp3Converter:   
    ...
    def convertToMp3(self,src,dst,bitRate,sampleRate):
        print(("mp3Converter:Convert","{}->{} :{}kbs,{}".format(src.getFileName(),dst.getFilePath(),bitRate,sampleRate)))
        self.m_node.info("mp3Converter:Convert","{}:{}:{} kbs:{}".format(src.getFileName(),dst.getFileName(),bitRate,sampleRate))
        ...
        if bitRate>0:
            params=params+["-b:a",str(bitRate)+"k"]
        if sampleRate>0:        
        ...
        try:
            ...
            return self.m_node.critical("mp3Converter:convert:call","unable to exec {}. error: {}".format([cmd]+params,e))
        except OSError as e:
            print("unable to exec",paramsFinal,"OS error",e)
            return self.m_node.critical("mp3Converter:convert:os","unable to exec {}. error: {}".format([cmd]+params,e))
        return True
    @staticmethod  
    def ffmpegPath():
...
def process(self):
    ...
    for c in inputs:
        for input in c.getUniversalPaths():
            ...
            res=mp3.convertToMp3(input,dst,int(self.bitRate),int(self.sampleRate)) and res
